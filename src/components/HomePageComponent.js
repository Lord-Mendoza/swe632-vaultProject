...
class HomePageComponent extends React.Component {
  ...
  render() {
    ...
    if (
      ...
      isNotAnEmptyObject(database["entries"])
    ) {
      ...
      if (isNotAnEmptyArray(sections)) {
        Object.values(sections).forEach((section) => {
          ...
          entryContents.push(
            <Row
              noGutters
              style={{ paddingBottom: ".5em", paddingLeft: "2em" }}
            >
      ...
      content = (
        <Segment raised inverted={darkMode} style={{ marginTop: "10px" }}>
          <div className="center-screen">
            <h2>
              Select one of your entries to the left to start viewing its
              information here.
            </h2>
          </div>
        </Segment>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="mr-auto">
              <NavDropdown id="nav-dropdown" title="Manage Vault">
                <NavDropdown.Item onClick={this.handleSelection}>
              ...
              <Nav.Link onClick={() => alert("clicked")}>
                Create New Entry
              </Nav.Link>
            </Nav>
